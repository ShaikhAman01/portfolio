        ...
        <section className="mx-auto grid max-w-[105rem] gap-12 px-5 pb-16 pt-12 md:px-8 md:pb-24 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <Reveal>
            <div className="mb-8 flex flex-wrap items-center gap-4">
              <span className="bg-[var(--on-surface)] px-5 py-3 text-lg font-bold tracking-[0.16em] text-[var(--surface-container-lowest)]">
                VERSION {project.version.replace("v", "")}
              </span>
              <span className="text-lg font-bold tracking-[0.18em] text-[var(--outline)]">
                STATUS: {project.status}
              </span>
            </div>
            <h1 className="break-words text-6xl font-black tracking-tight md:text-8xl xl:text-9xl">
              {project.title}
            </h1>
            <p className="mt-10 max-w-5xl text-2xl leading-[1.75] text-[var(--on-surface-variant)] md:text-3xl">
          ...
          <Reveal delay={0.1}>
            {/* FIXED: Shifted from aspect-square to aspect-video to prevent widescreen cropping */}
            <div className="relative aspect-video w-full overflow-hidden border border-[var(--outline-variant)] bg-zinc-950 p-2 shadow-[10px_10px_0_rgba(0,0,0,0.1)] dark:border-zinc-800">
              <Image
          ...
          {project.metrics.map(([label, value], index) => (
            <Reveal key={label} delay={index * 0.05}>
              <div className="group relative min-w-0 overflow-hidden border border-[var(--outline-variant)] bg-[var(--surface-container-lowest)] p-8 transition-colors hover:border-blue-700 dark:hover:border-emerald-400">
                ...
                <p className="mb-6 break-words text-xs font-black tracking-[0.2em] text-[var(--outline)] md:text-sm">
  {label}
</p>
                <p
                  className={`break-words text-3xl font-black leading-tight md:text-4xl ${label === "AVAILABILITY" ? "text-emerald-400" : ""}`}
                >
                  {value}
                </p>
        ...
        <section
          id="docs"
          className="mx-auto grid max-w-[105rem] gap-12 px-5 py-20 md:px-8 lg:grid-cols-[1.3fr_0.7fr]"
        >
          <div>
            <Reveal>
            ...
            <Reveal delay={0.08} className="mt-20">
              ...
              <div className="grid gap-4 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5">
                {project.stack.map((item) => (
                  <div
                    key={item}
                    className="min-w-0 flex items-center justify-center border border-[var(--outline-variant)] bg-[var(--surface-container-lowest)] px-3 py-5 text-center text-xs md:text-sm font-black tracking-[0.05em] uppercase break-words"
                    title={item.replace(":", ": ")}
                  >
                    {item}
                  </div>
            ...
            {["KEY_CHALLENGES", "KEY_LEARNINGS"].map((title, index) => (
              <Reveal key={title} delay={index * 0.08}>
                <div className="min-w-0 border border-[var(--outline-variant)] bg-[var(--surface-container-lowest)] p-8 shadow-[6px_6px_0_rgba(0,0,0,0.08)] dark:border-zinc-800">
                  <h2 className="break-words border-b border-[var(--outline-variant)] pb-6 text-2xl font-black tracking-[0.08em] md:text-3xl">
                    {title}
                  </h2>

                  <ul className="mt-8 space-y-8 text-lg leading-relaxed text-[var(--on-surface-variant)]">
                    {(title === "KEY_CHALLENGES"
                      ? project.challenges
                      : project.learnings
                    ).map((item, i) => (
                      <li
                        key={item}
                        className="grid grid-cols-[100px_1fr] gap-4 items-start break-words"
                      >
          ...
          <Reveal>
            <div className="flex min-w-0 flex-col gap-8 border border-[var(--outline-variant)] bg-[var(--surface-container-lowest)] p-8 shadow-[8px_8px_0_rgba(0,0,0,0.08)] md:flex-row md:items-center md:justify-between md:p-14">
              <div className="min-w-0">
                <p className="break-words text-2xl font-black tracking-[0.08em] md:text-3xl">
                  SYSTEM_OPERATOR_ONLINE
                </p>
                <p className="mt-4 text-xl text-[var(--on-surface-variant)]">
              ...
              <div className="flex min-w-0 flex-col gap-4 sm:flex-row">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="bg-black px-8 py-5 text-center text-base font-black tracking-[0.12em] text-white transition hover:bg-blue-700 dark:bg-white dark:text-black dark:hover:bg-emerald-400 md:text-lg"
                >
                  VIEW_SOURCE
                </a>
                ...
                {hasLiveLink && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="border border-black bg-[var(--surface-container-lowest)] px-8 py-5 text-center text-base font-black tracking-[0.12em] text-black transition hover:bg-black hover:text-white dark:border-white dark:text-white dark:hover:bg-white dark:hover:text-black md:text-lg"
                  >
                    LAUNCH_LIVE_APP →
                  </a>
                )}
                ...
                <Link
                  href="/#contact"
                  className="border border-[var(--outline-variant)] px-8 py-5 text-center text-base font-black tracking-[0.12em] transition hover:bg-[var(--on-surface)] hover:text-[var(--surface-container-lowest)] md:text-lg"
                >
                  CONTACT_ENGINEER
                </Link>
              </div>
